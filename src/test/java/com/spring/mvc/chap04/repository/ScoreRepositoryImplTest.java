...
/*
        단위 테스트 ( Unit test )
        기능 하나하나 다 점검 할려면 테스트 시나리오가 필요하다
        - 단언( Assertion ) 기법  ex ) ~~ 한다.
        -
 ...
 */
    ...
    void findAllTest(){
        ...
        // 나는 스코어리시트의 사이즈가 3인 것이 참이라고 단언한다
        // assertTrue(scoreList.size() == 3);
        // assertEquals(3, scoreList.size() == 3);
        // 스코어 리스트의 첫번째 객체의 이름이 뽀로로라고 단언한다
        assertEquals("뽀로로", scoreList.get(0).getName());
    }

    @Test
    @DisplayName("나는 저장소에서 findByStuNum을 호출하여 학번이 2인 학생을 조회하면 " +
    ...
    void findOneTest() {
     ...
     @Test
    @DisplayName("나는 저장소에서 findByStuNum을 호출하여 학번이 99인 학생을 조회하면 null이 리턴될 것이다.")
    void findOneTest2() {
        // given
        int stuNum = 99;
        // when
    ...
    @Test
    @DisplayName("저장소에서 학번이 2인 학생을 삭제한 후에 리스트를 전체 조회해보면 " +
            "성적의 개수가 2개일 것이고 다시 2번학생을" +
            "조회 했을때 Null이 반환되어야 한다")
    void deleteTest(){
        // given
        int stuNum = 2;
        // when
...
//        }
    ...
    @Test
    @DisplayName("새로운 성적 정보를 save를 통해 추가하면" +
            "목록의 개수가 4개여야 한다.")
    void seveTest() {
        //given
        ...
        scoreList.forEach(score1 -> System.out.println(score1));
    }
